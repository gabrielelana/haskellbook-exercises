...
1. Given the following function
   ...
   quickCheckWithResult stdArgs {chatty=False} property_HalfIdentity >>= print . output
   #+END_SRC
...
2. For an ordered list the following property should hold
   ...
   quickCheckWithResult stdArgs {chatty=False} property_OrderedListProperty >>= print . output
   #+END_SRC
 ...
 3. Test the associative and commutative propertys of addition
    ...
    quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary associativeProperty) >>= print . output
    quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary commutativeProperty) >>= print . output
    #+END_SRC
...
5. Tests the laws involving ~quot/rem~ and ~div/mod~
   ...
   quickCheckWithResult (stdArgs {chatty=False}) (forAll (arbitrary :: Gen (Int, NonZero Int)) (\(x, NonZero y) -> quotAndRemLaw x y)) >>= print . output
   quickCheckWithResult (stdArgs {chatty=False}) (forAll (arbitrary :: Gen (Int, NonZero Int)) (\(x, NonZero y) -> divAndModLaw x y)) >>= print . output
   #+END_SRC
...
7. Test that reversing the reverse of a list is equal to identity
   ...
   quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary reverseOfReverseProperty) >>= print . output
   #+END_SRC
...
8. Write a property for the definition of ~($)~
   ...
   quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary applicationOperatorDefinition) >>= print . output
   #+END_SRC
   ...
   quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary composeOperatorDefinition) >>= print . output
   #+END_SRC
   ...
   quickCheckWithResult (stdArgs {chatty=False}) (forAll functionGenerator applicationOperatorDefinition) >>= print . output
   #+END_SRC
...
9. See if the following functions are equal
   ...
   quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary equalityProperty) >>= print . output
   #+END_SRC
...
10. Is that so?
    ...
    quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary property) >>= print . output
    #+END_SRC
...
quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary squareIdentity) >>= print . output
quickCheckWithResult (stdArgs {chatty=False}) (forAll arbitrary squareIdentity') >>= print . output
#+END_SRC
...
1. Test the function ~fillInCharacters~
   ...
   quickCheckOutput p = quickCheckWithResult (stdArgs {chatty=False}) p >>= print . (reverse . dropWhile Data.Char.isSpace . reverse) . output

   quickCheckOutput fillInCharacterIsMonotone
   #+END_SRC
...
quickCheckWithResult (stdArgs {chatty=False})
  (forAll shiftGenerator
    (\n -> forAll wordGenerator
      (\w -> caesarRoundTrip n w))) >>= print . output
:}
...
quickCheckWithResult (stdArgs {chatty=False})
  (forAll wordGenerator
    (\keyword -> forAll wordGenerator
      (\word -> vigenereRoundTrip keyword word))) >>= print . output
:}
#+END_SRC
